...
# Database Operations
class TaskDatabase:
    def __init__(self, db_name='Tasks TO DO.db'):
...
class TaskManager:
    ...
    def create_widgets(self):
        ...
        self.task_field = Entry(
            self.functions_frame,
            font=("Arial", "14"),
            width=42, fg="orange", bg="orange"
        )
        self.task_field.place(x=180, y=30)

        Button(
            self.functions_frame, text="ADD TASK", width=11,
            bg='#D4AC0D', font=("arial", "12",),
            command=self.add_task
        ).place(x=18, y=80)

        Button(
            self.functions_frame, text="REMOVE TASKS", width=18,
            bg='#D4AC0D', font=("arial", "10",),
            command=self.delete_task
        ).place(x=240, y=80)

        Button(
            self.functions_frame, text="DELETE TASKS", width=16,
            bg='#D4AC0D', font=("arial", "12",),
            command=self.delete_all_tasks
        ).place(x=460, y=80)

        Button(
            self.functions_frame, text="CLOSE", width=52,
            bg='#D4AC0D', font=("arial", "14",),
            command=self.close
        ).place(x=17, y=330)

        self.task_listbox = Listbox(
            self.functions_frame, width=50, height=9,
            selectmode='SINGLE',
            bg="ORANGE", fg="ORANGE",
    ...
    def add_task(self):
        ...
        if not task:
            messagebox.showinfo('Error', 'AREA is Empty.')
            return

        if task in self.tasks:
            messagebox.showinfo('Error', 'AREA already exists.')
            return
